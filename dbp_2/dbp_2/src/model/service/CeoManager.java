...
public class CeoManager {
	...
	public int create(Ceo ceo) throws SQLException, ExistingUserException {
		if (ceoDAO.existingUser(ceo.getCeoId()) == true) {
			throw new ExistingUserException(ceo.getCeoId() + "는 존재하는 이메일입니다.");
		}
	...
	public int remove(String ceoId) throws SQLException {
		return ceoDAO.remove(ceoId);
	}

	public Ceo findUser(String ceoId) throws SQLException, UserNotFoundException {
		Ceo ceo = ceoDAO.findUser(ceoId);

		if (ceo == null) {
			throw new UserNotFoundException(ceoId + "는 존재하지 않는 이메일입니다.");
		}
		return ceo;
	}
